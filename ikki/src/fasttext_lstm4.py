...
from keras.layers import Bidirectional, GlobalMaxPool1D, GlobalAveragePooling1D, TimeDistributed, Permute, Reshape, Lambda, RepeatVector, Multiply, Concatenate, SpatialDropout1D, Conv1D
...
def build_lstm_stack_model(logdir='attention'):
    # Bidirectional-LSTM
    inp = Input(shape=(window_length, 300))
    inp_dr = SpatialDropout1D(0.15)(inp)
    l_lstm = Bidirectional(GRU(50, return_sequences=True, dropout=0.1, recurrent_dropout=0.01))(inp)
    l_conv2 = Conv1D(128, kernel_size = 2, padding = "valid", kernel_initializer = "he_uniform")(l_lstm)
    l_conv3 = Conv1D(128, kernel_size = 3, padding = "valid", kernel_initializer = "he_uniform")(l_lstm)
    ...
    l_conv = Concatenate(axis=1)([l_conv2, l_conv3, l_conv4, l_conv5])
    x_gmp = GlobalMaxPool1D()(l_conv)
    x_gap = GlobalAveragePooling1D()(l_conv)
    x = Concatenate()([x_gmp, x_gap])
    x = Dropout(0.35)(x)

    l_lstm_reverse = Bidirectional(GRU(50, return_sequences=True, go_backwards=True, dropout=0.1, recurrent_dropout=0.01))(inp)
    l_conv2_reverse = Conv1D(128, kernel_size = 2, padding = "valid", kernel_initializer = "he_uniform")(l_lstm_reverse)
    l_conv3_reverse = Conv1D(128, kernel_size = 3, padding = "valid", kernel_initializer = "he_uniform")(l_lstm_reverse)
    l_conv4_reverse = Conv1D(128, kernel_size = 4, padding = "valid", kernel_initializer = "he_uniform")(l_lstm_reverse)
    l_conv5_reverse = Conv1D(128, kernel_size = 5, padding = "valid", kernel_initializer = "he_uniform")(l_lstm_reverse)
    #l_dense = TimeDistributed(Dense(50, activation="relu"))(l_lstm)
    l_conv_reverse = Concatenate(axis=1)([l_conv2_reverse, l_conv3_reverse, l_conv4_reverse, l_conv5_reverse])    #l_dense_reverse = TimeDistributed(Dense(50, activation="relu"))(l_lstm_reverse)
    ...
    x_reverse = Dropout(0.35)(x_reverse)
...
"""
# Filename to save
saving_filename = 'fasttext_lstm4_cv'

# Define KFold and random state
random_state = 3332222
n_splits = 5
