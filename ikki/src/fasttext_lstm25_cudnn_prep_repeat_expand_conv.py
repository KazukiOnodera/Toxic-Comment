...
from keras.layers import Conv2D, Lambda, SpatialDropout1D
...
def build_lstm_stack_model(logdir='attention'):
    ...
    inp_expand = Reshape((window_length, 300, 1))(inp_dr)
    ...
    conv2_pool = MaxPooling2D(pool_size=(5, 1), strides=None, padding='valid')(conv2_bn)

    conv3 = Conv2D(256, kernel_size=(4, 300), padding='valid', activation='elu')(inp_expand)
    conv4 = Conv2D(256, kernel_size=(4, 1), padding='valid', activation='elu')(conv3)
    conv4_bn = BatchNormalization()(conv4)
    conv4_pool = MaxPooling2D(pool_size=(4, 1), strides=None, padding='valid')(conv4_bn)

    conv5 = Conv2D(256, kernel_size=(3, 300), padding='valid', activation='elu')(inp_expand)
    conv6 = Conv2D(256, kernel_size=(3, 1), padding='valid', activation='elu')(conv5)
    conv6_bn = BatchNormalization()(conv6)
    conv6_pool = MaxPooling2D(pool_size=(3, 1), strides=None, padding='valid')(conv6_bn)

    conv7 = Conv2D(256, kernel_size=(1, 300), padding='valid', activation='elu')(inp_expand)
    conv8 = Conv2D(256, kernel_size=(1, 1), padding='valid', activation='elu')(conv7)
    conv8_bn = BatchNormalization()(conv8)
    conv8_pool = MaxPooling2D(pool_size=(2, 1), strides=None, padding='valid')(conv8_bn)

    x = Concatenate(axis=1)([conv2_pool, conv4_pool, conv6_pool, conv8_pool])
    x = Flatten()(x)

    inp_feat = Input(shape=(len(eng_feat_cols),))
    x = Concatenate()([x, inp_feat])

    x = Dense(196, activation="elu")(x)
    x = Dropout(0.2)(x)
...
train['comment_text'] = train['comment_text'].apply(lambda x: x.split())
test['comment_text'] = test['comment_text'].apply(lambda x: x.split())

# commentを拡張する
def expand_comment(comment):
    comment_len = len(comment)
    if comment_len > window_length:
        return comment
    elif comment_len == 0:
        return comment
    expand_num = window_length // comment_len + 1
    comment += comment * expand_num
    return comment[:window_length]

train['comment_text'] = train['comment_text'].apply(expand_comment)
test['comment_text'] = test['comment_text'].apply(expand_comment)

"""
Training and evaluating with cross-validation
"""
# Filename to save
saving_filename = 'fasttext_lstm19_cudnn_prep_repeat_cv'
