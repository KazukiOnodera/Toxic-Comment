...
def build_lstm_stack_model(logdir='attention'):
    ...
    inp_dr = SpatialDropout1D(0.3)(inp)
    l_lstm = Bidirectional(CuDNNGRU(128, return_sequences=True))(inp)
    ...
    inp_reverse = Lambda(lambda x: K.reverse(x, axes=1))(inp)
    l_lstm_reverse = Bidirectional(CuDNNGRU(128, return_sequences=True))(inp_reverse)
    ...
    x_reverse = Dropout(0.5)(x_reverse)
    ...
    dense_per_class = []
    for i in classes:
        x_dense = Dense(48, activation="elu")(x)
    ...
    model.compile(loss='binary_crossentropy', optimizer=Adamax(lr=1e-3, decay=5e-5), metrics=['accuracy'])
...
"""
# Filename to save
saving_filename = 'fasttext_lstm5_cudnn_cv'
